  ...
  const [messages, setMessages] = React.useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "Olá! Sou a XpIA, a assistente IA da XPay. Posso ajudar com pagamentos, FX, onboarding, integração API e muito mais. Como posso ajudar?",
      timestamp: Date.now(),
    },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const fallbackMsg: Message = {
        id: `a-${Date.now()}`,
        role: "assistant",
        content: "Estou temporariamente indisponível para chat em tempo real. Para urgências, contacte-nos via Telegram (@XPay_Expert), WhatsApp (+55 62 99288-7416) ou email (contact@xpay.expert).",
        timestamp: Date.now(),
      };
