...
import { useApiKeys, useCreateApiKey, useDeleteApiKey, useStores } from "@/hooks/use-queries";
...
export default function ApiKeysPage() {
  ...
  function handleCreate() {
    createMutation.mutate(
      { name, environment, scopes, storeId },
      {
    ...
    <div className="flex flex-col gap-6">
      ...
      <motion.div {...fadeUp} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-amber-500/25 bg-amber-500/8 px-3 py-2 text-xs text-amber-300">
          ...
          <span>Never share live secret keys. Rotate immediately on suspected exposure.</span>
        </div>
      </motion.div>
      ...
      <Card className="border-border/60 bg-card/60 p-5 backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{filtered.length} keys &middot; {keys.filter((k) => k.environment === "live").length} live</p>
          </div>
        </div>
        ...
        {isError ? (
          <div className="flex flex-col items-center gap-2 py-8">
            ...
            <Button variant="outline" className="mt-2 gap-1.5" onClick={() => refetch()}>
          ...
          <EmptyState
            ...
            description="Create your first key to start integrating XPay."
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            >
                              {deleteMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Revoke key"}
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
