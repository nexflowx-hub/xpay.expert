...
export default function TransactionsPage() {
  ...
  if (isError) {
    return (
      <div className="flex flex-col gap-6">
        <PageHeader title="Transactions" description="Full transaction ledger with advanced filters and search." />
        <ErrorState
          message="Failed to load transactions. The backend may be unreachable."
          onRetry={() => refetch()}
        />
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Transactions"
        description="Full transaction ledger with advanced filters and search."
      />
          ...
          <Popover open={dateOpen} onOpenChange={setDateOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" className="gap-1.5">
                ...
                {filters.from || filters.to ? "Date set" : "Date range"}
              </Button>
            </PopoverTrigger>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border/60 text-left text-xs text-muted-foreground">
                ...
                <TableHead className="text-xs font-medium">Fee</TableHead>
                  ...
                  : transactions.map((tx) => (
                      <TableRow key={tx.id} className="border-border/30">
                        ...
                        <TableCell className="text-right text-xs text-muted-foreground">
                          {formatDate(tx.createdAt, { withTime: true })}
                        </TableCell>
                      </TableRow>
                    ))}
            </TableBody>
                ...
                <motion.div key={tx.id} {...fadeUp}>
                  <Card className="border-border/60 bg-card/60 p-4 backdrop-blur-xl">
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <span>{formatDate(tx.createdAt, { withTime: true })}</span>
                    </div>
                  </Card>
                </motion.div>
              ))}
