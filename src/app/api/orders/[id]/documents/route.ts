...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const upstream=await fetch(`${API}/api/v1/expert/orders/${encodeURIComponent(id)}/documents`,{method:'POST',headers:{Authorization:`Bearer ${auth}`,'Content-Type':'application/json'},body:JSON.stringify(body),cache:'no-store'});
