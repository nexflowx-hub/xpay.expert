...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const upstream=await fetch(`${API}/api/v1/expert/orders/${encodeURIComponent(id)}/intake`,{headers:{Authorization:`Bearer ${auth}`,Accept:'application/json'},cache:'no-store'});
